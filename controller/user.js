...
//     try {
//         const { fullName, email, phoneNumber, password, confirmPassword, dateOfBirth, gender, role } = req.body;
...
//             'INSERT INTO user (fullName, email, phoneNumber, password, dateOfBirth, gender, role) VALUES (?, ?, ?, ?, ?, ?, ?)',
//             [fullName, email, phoneNumber, hashedPassword, dateOfBirth, gender, role]
//         );
...
const login = async (req, res) => {
    try {
        ...
        // Check if user exists
        const [user] = await db.query('SELECT id, email, role, password FROM users WHERE email = ?', [email]);
        if (user.length === 0) {
